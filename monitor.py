...
# 服务器的JVM的pid 并去掉空格
bash_pid = "/acme_dbgo/jdk8/jdk1.8.0_161/bin/jps | grep 'Bootstrap' | awk '{print $1}'"
jvm_pid = os.popen(bash_pid).read().strip()

# 获取JVM中存活得对象
bash_jmap = "/acme_dbgo/jdk8/jdk1.8.0_161/bin/jmap -histo:live " + jvm_pid + " | head -13 "
jvm_instance_arr = get_text_sh(bash_jmap)

# JVM堆信息
bash_jmap_heap = "/acme_dbgo/jdk8/jdk1.8.0_161/bin/jmap -heap " + jvm_pid
jvm_heap_arr = get_text_sh(bash_jmap_heap)

# gc统计，采样时间间隔为250ms，采样数为4
bash_gc = "/acme_dbgo/jdk8/jdk1.8.0_161/bin/jstat -gc " + jvm_pid + " 250 4 "
